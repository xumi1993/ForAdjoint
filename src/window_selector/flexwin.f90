module flexwin
  ...
  contains
    procedure :: init => initialize, select_windows
    procedure, private :: calculate_criteria, reject_on_minima_water_level, &
...
contains
  ...
  subroutine calculate_criteria(this, cand)
    class(win_sel_type), intent(in) :: this
  ...
  subroutine select_windows(this)
    ...
    do i = 1, n_peaks
       if (stalta(peaks(i)) < win_config_global%water_level) cycle
    ...
    if (win_config_global%resolution_strategy == 1) then
    ...
    elseif (win_config_global%resolution_strategy == 2) then ! Default to interval_scheduling
      ...
      error stop 'Unknown resolution strategy in window selection.'
    end if
    ...
    do i = 1, n_selected
       ...
       this%twin(i, 1) = real(candidates(j)%left_idx - 1, kind=dp) * this%dt
       this%twin(i, 2) = real(candidates(j)%right_idx - 1, kind=dp) * this%dt
  ...
  subroutine reject_on_minima_water_level(this, candidates, n_cand, stalta, troughs)
    ...
    do i = 1, n_cand
       ...
       waterlevel_midpoint = win_config_global%c_0 * win_config_global%water_level
